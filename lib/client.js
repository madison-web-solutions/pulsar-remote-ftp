  ...
  doConnect() {
    ...
    switch (self.info.protocol) {
      ...
      case 'sftp': {
        ...
        if (!self.info.pass && !self.info.privatekey) {
          // See if ~/.ssh/id_rsa exists and use it by default if it does
          if (FS.existsSync(resolveHome('~/.ssh/id_rsa'))) {
            self.info.privatekey = '~/.ssh/id_rsa';
          }
        }
